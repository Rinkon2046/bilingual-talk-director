...
description: >-
  双语口播导演:用 HyperFrames(HeyGen 开源、写 HTML 渲染视频)把竖屏出镜口播包装成短视频成片。
  适用场景:给口播录像配顶部章节进度条、底部中英双语强字幕、关键词橙色高亮、暗色引用/产品卡片和可选转场大标题。
  当用户要"做成小红书/抖音风格口播视频""给真人讲解加双语字幕和章节条""复刻口播强字幕风格""用 HyperFrames 出竖屏短视频"时使用本技能。
license: Apache-2.0
version: 1.0.0
---
...
3. **填字幕**。按字幕表增删 `.cue` 块:中文放 `.zh`、英文放 `.en`,关键词用 `<span class="hl">…</span>` 包起来(中英都包,视觉一致)。给每个 cue 一个唯一 id。字幕默认**尽量保持一行**:每条 cue 要短、跟语音节奏切分,不要为了塞长句而自动换成两行;如果单行略超宽,优先动态缩小字号或继续拆短 cue。
...
- 字幕应单行优先:中文 `.zh` 和英文 `.en` 使用 `white-space:nowrap`,并在超宽时降低字号适配;不要默认换行。

## 设计规则速记(细节见 STYLE.md)
- 高亮色仅一种:橙 `#EC6A35`,只给"关键词";其余中文纯白、英文用灰 `#C7C4BD`。
- 中文用 `Noto Sans SC` 900 加粗大字;英文/标签/卡片引文用等宽 `JetBrains Mono`,带字距。
- 双语字幕尽量单行显示:中文一行、英文一行。长句先按语音切成更短 cue;确实放不下时动态缩小字号,不要自动另起一行。
